...
summary.tobitModel <- function(object, method = c("analytic", "numeric"), ...) {
  ...
  if (left == 0 && is.infinite(right)) {
    ...
    # MARGINAL EFFECTS IN CONSISTENT ORDER: probability → conditional → unconditional
    ME_prob   <- beta_hat * (phi / sigma_hat)                                   # dP(y>0)/dx_j
    C_factor  <- 1 - lambda * (z + lambda)                                      # scalar multiplier
    ME_cond   <- beta_hat * C_factor                                            # dE[y|y>0]/dx_j
    ME_uncond <- beta_hat * Phi                                                 # dE[y]/dx_j
    
    vcov_all  <- vcov(result)
    k         <- length(beta_hat)
    
    if (method == "analytic") {
      # ANALYTIC GRADIENTS IN CONSISTENT ORDER: probability → conditional → unconditional
      
      # 1. PROBABILITY MARGINAL EFFECTS GRADIENT:
      ...
      for (j in 1:k) {
        ...
        grad_prob[j, k + 1] <- grad_prob[j, k + 1] + beta_hat[j] * dT_dsigma
      }
      
      # 2. CONDITIONAL MARGINAL EFFECTS GRADIENT:
      ...
      for (j in 1:k) {
        ...
        grad_cond[j, k + 1] <- grad_cond[j, k + 1] + beta_hat[j] * dC_dsigma
      }
      
      # 3. UNCONDITIONAL MARGINAL EFFECTS GRADIENT
      ...
      # NUMERIC GRADIENTS IN CONSISTENT ORDER: probability → conditional → unconditional:
      # 1. PROBABILITY MARGINAL EFFECTS GRADIENT:
    ...
    # STANDARD ERRORS IN CONSISTENT ORDER:
    ...
    # OUTPUT IN CONSISTENT ORDER: probability → conditional → unconditional
...
# --- Predict Method (UPDATED ORDER: probability → conditional → unconditional) ---
predict.tobitModel <- function(object, newdata = NULL, type = c("probability", "conditional", "response")) {
  type <- match.arg(type)
  ...
  # CONSISTENT ORDER: probability → conditional → unconditional:
  switch(type,
         probability = { # P(y > 0)
           pnorm(z)
         },
